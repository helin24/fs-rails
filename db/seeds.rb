...
require_relative '../lib/utilities/diagram_selector'
require 'csv'
...
diagram_color = "#DCDCDDDDDEDE"
pdf_images_path = Rails.root.to_s + '/app/assets/images/pictures/'
diagram_file_path = Rails.root.to_s + '/app/assets/images/diagrams/'
...
(mitf_start_page..mitf_end_page).each do |p|
	...
	diagram_path = 'field_move_' + p.to_s + '.png'
	FieldMove.create(name: field_move_name, test_level_id: TestLevel.find_by(name: test_level_name).id, description: field_move_description, image: diagram_path)
end

# Run once to get cropped diagrams into diagrams folder

# (mitf_start_page..mitf_end_page).each do |p|
# 	pdf_file_name = file + '_' + p.to_s + '.png'
# 	selector = DiagramSelector.new(pdf_images_path + pdf_file_name)
# 	diagram = selector.crop_by_color(diagram_color)

# 	diagram_file_name = "field_move_" + p.to_s + '.png'
# 	diagram.write(diagram_file_path + diagram_file_name)
# end
